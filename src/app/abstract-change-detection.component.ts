import {AfterViewInit, ChangeDetectionStrategy, ChangeDetectorRef, DestroyRef, Directive, ElementRef, HostBinding, inject, Input, NgZone, OnChanges, signal, SimpleChanges, ViewChild} from '@angular/core';
...
export abstract class AbstractChangeDetectionComponent implements AfterViewInit,
  ...
  @ViewChild('hidden_button', {static: true})
  private _hiddenButton!: ElementRef<HTMLButtonElement>;
  @ViewChild('action_list', {static: true})
  private _actionSelect!: ElementRef<HTMLSelectElement>;
  ...
  public cdStatus: 'traversal'|'dirty'|'dirty consumer'|null = null;
    ...
    this._zone.runOutsideAngular(() => {
      timer(0, 200)
          ...
          .subscribe((cdStatus) => {
            ...
            this._ngMarked.nativeElement.classList.add(
                'tag', cdStatus?.replace(' ', '-'));
          });
      ...
      fromEvent(this._executeButton.nativeElement, 'click')
          ...
          .subscribe(() => {
            switch (this._actionSelect.nativeElement.value) {
              case 'click':
